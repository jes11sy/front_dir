...
export default function CashLayout({ children }: { children: React.ReactNode }) {
  ...
  const navigateToTab = useCallback(
    (href: string) => {
      // Без принудительного скролла вверх — визуально переход мягче.
  ...
  const navigateBySwipe = useCallback(
    (direction: 'left' | 'right') => {
      ...
      navigateToTab(tabs[nextIndex]!.href)
    },
  ...
  const handleTouchEnd = useCallback(
    (e: React.TouchEvent) => {
      ...
      // Горизонтальный свайп: уверенный жест, вертикальный скролл игнорируем.
  ...
  useEffect(() => {
    // Прогреваем соседние вкладки, чтобы переключение не выглядело как полный reload.
    tabs.forEach((tab) => {
  ...
  return (
    <div
      ...
      className={`min-h-full transition-colors duration-300 ${isDark ? 'bg-[#111113]' : 'bg-[#f5f5f7]'}`}
    >
      <div className="px-4 pb-6 pt-0 md:pt-6">
        <div className="w-full">
          <div className={`transition-colors duration-300 ${isDark ? 'bg-[#111113]' : 'bg-[#f5f5f7]'}`}>
            {/* Табы: на мобилке закреплены при скролле */}
            <div
              className={`sticky z-30 -mx-4 mb-4 px-4 py-2 backdrop-blur-xl top-[calc(4rem+env(safe-area-inset-top,0px))] md:static md:top-auto md:z-auto md:mx-0 md:border-0 md:px-0 md:py-0 ${
                isDark
