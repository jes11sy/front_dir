    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{backgroundColor: '#114643'}}>
        <div className="text-center">
          ...
          <button 
            onClick={() => router.push('/cash/income')}
    ...
    <div className="min-h-screen" style={{backgroundColor: '#114643'}}>
      <div className="container mx-auto px-4 py-4 sm:py-8">
        <div className="max-w-2xl mx-auto">
          <div className="backdrop-blur-lg shadow-2xl rounded-lg sm:rounded-2xl p-4 sm:p-8 border" style={{backgroundColor: '#15282f', borderColor: '#114643'}}>
            ...
            <div className="space-y-3 sm:space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-gray-300 text-sm sm:text-base">ID:</span>
                <span className="text-white font-medium text-sm sm:text-base">{transaction.id}</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-gray-300 text-sm sm:text-base">Тип:</span>
                <span 
              ...
              <div className="flex justify-between items-center">
                <span className="text-gray-300 text-sm sm:text-base">Город:</span>
                <span className="text-white text-sm sm:text-base">{transaction.city || 'Не указан'}</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-gray-300 text-sm sm:text-base">Сумма:</span>
                <span className="text-white font-semibold text-green-400 text-sm sm:text-base">
                  {Number(transaction.amount).toLocaleString()} ₽
                </span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-gray-300 text-sm sm:text-base">Назначение платежа:</span>
                <span className="text-white text-sm sm:text-base text-right max-w-[50%]">{transaction.paymentPurpose || 'Не указано'}</span>
              </div>

              <div className="flex justify-between items-start">
                <span className="text-gray-300 text-sm sm:text-base">Комментарий:</span>
                <span className="text-white text-right max-w-[50%] text-sm sm:text-base">
                  {transaction.note || 'Нет комментария'}
                </span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-gray-300 text-sm sm:text-base">Дата создания:</span>
                <span className="text-white text-sm sm:text-base">{formatDate(transaction.dateCreate)}</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-gray-300 text-sm sm:text-base">Создатель:</span>
                <span className="text-white text-sm sm:text-base text-right max-w-[50%]">{transaction.nameCreate || 'Не указан'}</span>
              </div>
              ...
              {transaction.receiptDoc && (
                <div>
                  ...
                  {(() => {
                    ...
                    if (isImage) {
                      return (
                        <div className="bg-gray-800 rounded-lg p-3 sm:p-4 border border-gray-600 mt-3">
                          <div className="flex flex-col items-center">
                            <img 
                              src={receiptUrl || ''}
                              alt="Чек"
                              className="w-32 h-32 sm:w-48 sm:h-48 object-cover rounded border border-gray-600 cursor-pointer hover:opacity-80 transition-opacity"
                              onClick={async () => {
                            ...
                            />
                            <p className="text-xs text-gray-400 mt-2 text-center">Нажмите на изображение для открытия в новой вкладке</p>
                          </div>
                        </div>
                      )
                    } else {
                      return (
                        <div className="bg-gray-800 rounded-lg p-4 sm:p-6 text-center border border-gray-600 mt-3">
                          <div className="text-gray-400 mb-4">
                            ...
                            <div className="text-base sm:text-lg font-medium">Документ</div>
                            <div className="text-xs sm:text-sm text-gray-500">{transaction.receiptDoc.split('/').pop()}</div>
                          </div>
                          <button 
                            className="px-3 py-2 sm:px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors text-sm sm:text-base"
                            onClick={async () => {
                              if (transaction.receiptDoc) {
                                const signedUrl = await getSignedUrl(transaction.receiptDoc)
                                window.open(signedUrl, '_blank')
